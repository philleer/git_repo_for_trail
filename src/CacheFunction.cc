...
// github::Plane::Plane(cv::Vec3f point, cv::Vec3f normal): point(point), normal(normal) {
// 	coeff[0] = -normal[0] / normal[2];
// 	coeff[1] = -normal[1] / normal[2];
// 	coeff[2] = (normal[0]*point[0]+normal[1]*point[1]+normal[2]*point[2])/normal[2];
// }

// inline float github::Plane::operator[](int idx) const { return coeff[idx]; }
// inline cv::Vec3f github::Plane::operator()() { return coeff; }
// inline cv::Vec3f github::Plane::getPoint() { return point; }
// inline cv::Vec3f github::Plane::getNormal() { return normal; }
// inline	cv::Vec3f github::Plane::getCoeff() { return coeff; }

// Plane github::Plane::viewTransform(int x, int y, int sign, int &qx, int &qy) {
...
// bool github::check_image(const cv::Mat &img, std::string name="Image") {
// 	if (!img.data) {
...
// bool github::check_dimensions(const cv::Mat &img1, const cv::Mat &img2) {
...
// void github::compute_gray_gradient(const cv::Mat &img, cv::Mat &grad) {
// 	int scale = 1, delta = 0;
...
// github::PatchMatch::PatchMatch(float alpha, float gamma, float tc, float tg) :
// 	alpha(alpha), gamma(gamma), tc(tc), tg(tg) {}

// cv::Mat github::PatchMatch::getLeftDisparityMap() const { return this->disps[0]; }
// cv::Mat github::PatchMatch::getRightDisparityMap() const { return this->disps[1]; }

// void github::PatchMatch::precompute_pixels_weights(const cv::Mat &img, cv::Mat &weights, int ws)
// {
// 	int ws_half = ws / 2;
...
// void github::PatchMatch::initialize_random_planes(Matrix2D<Plane> &planes, float max_d) {
...
// void github::PatchMatch::evaluate_planes_cost(int idx) {
// 	#pragma omp parallel for
// 	for (int y = 0; y < rows; ++y) {
// 		for (int x = 0; x < cols; ++x) {
...
// float github::PatchMatch::plane_match_cost(const Plane &p, int cx, int cy, int ws, int idx)
// {
// 	int sign = -1+2*idx;
// 	float cost = 0;
// 	int half = ws/2;
...
// float github::PatchMatch::dissimilarity(const cv::Vec3f &pp, const cv::Vec3f &qq,
...
// void github::PatchMatch::process(int iterations, bool reverse) {
...
// void github::PatchMatch::process_pixel(int x, int y, int idx, int iter) {
// 	spatial_propagation(x, y, idx, iter);
// 	plane_refinement(x, y, idx, MAX_DISPARITY/2, 1.0f, 0.1f);
// 	view_propagation(x, y, idx);
// }

// void github::PatchMatch::spatial_propagation(int x, int y, int idx, int iter) {
...
// void github::PatchMatch::plane_refinement(int x, int y, int idx, float max_delta_z,
...
// void github::PatchMatch::planes_to_disparity(const Matrix2D<Plane> &planes, cv::Mat &disp)
// {
// 	#pragma omp parallel for
// 	for (int x = 0; x < cols; ++x) {
// 		for (int y = 0; y < rows; ++y) {
...
// void github::PatchMatch::view_propagation(int x, int y, int idx) {
...
// void github::PatchMatch::set(const cv::Mat &img1, const cv::Mat &img2) {
// 	this->rows = img1.rows;
// 	this->cols = img1.cols;
// 	this->views[0] = img1;
// 	this->views[1] = img2;
...
// void github::PatchMatch::postProcess() {
...
// void github::PatchMatch::fill_invalid_pixels(int y, int x, Matrix2D<Plane> &planes,
...
// void github::PatchMatch::weighted_median_filter(int cx, int cy, cv::Mat &disp, 

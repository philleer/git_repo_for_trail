...
void Thread::Start() {
    ...
    if (started_ && !finished_) {
        std::cerr << "started_ true and finished_ false!\n";
        exit(EXIT_FAILURE);
    }
    Wait();
    timer_.Restart();
...
void Thread::AddCallback(const int id, const std::function<void()>& func) {
    ...
    if (callbacks_.count(id) <= 0) {
        std::cerr << "Callback not registered! id num in callbacks_ <= 0!\n";
        exit(EXIT_FAILURE);
    }
...
ThreadPool::ThreadPool(const int num_threads): stopped_(false),

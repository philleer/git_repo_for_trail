#ifndef SRC_UTIL_CACHE_H
#define SRC_UTIL_CACHE_H
        ...
        max_num_elems_(max_num_elems), getter_func_(getter_func)
{
    if (getter_func == nullptr) {
        std::cerr << "getter_func is invalid!\n";
        exit(EXIT_FAILURE);
    }
    if (max_num_elems <= 0) {
        std::cerr << "max_num_elems = " << max_num_elems << " invali!\n";
        exit(EXIT_FAILURE);
    }
}
...
{
    if (max_num_bytes_ <= 0) {
        std::cerr << "max_num_bytes_ invalid!\n";
        exit(EXIT_FAILURE);
    }
}
                                                    ...
                                                    value_t&& value)
{
    ...
    elems_num_bytes_.insert(std::pair<key_t, size_t>(key, num_bytes));

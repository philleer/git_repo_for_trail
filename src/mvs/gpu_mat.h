#ifndef _SRC_MVS_GPU_MAT_H_
#define _SRC_MVS_GPU_MAT_H_
...
// #define CUDA_SAFE_CALLg(error) CudaSafeCallg(error, __FILE__, __LINE__)

// void CUDA_SAFE_CALLg(const cudaError_t error, const std::string& file,
//                   const int line) {
//     if (error != cudaSuccess) {
//         sprintf(std::cerr, "CUDA error at %s:%i - %s\n",
//                 file.c_str(), line,
...
GpuMat<T>::GpuMat(const size_t width, const size_t height, const size_t depth)
    : array_(nullptr),
      ...
      height_(height),
      depth_(depth) {
  // CUDA_SAFE_CALLg(cudaMallocPitch((void**)&array_ptr_, &pitch_,
